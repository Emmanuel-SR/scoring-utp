...
select pr.professor_id, count(*) as scorings, avg(sc.facility) as facility , avg(sc.`help`) as `help`,
 ...
 select pr.professor_id,CONCAT(pr.first_name,' ',pr.last_name) AS professor_full_name, IF(count(*) > 0, count(*), null) as scorings, avg(sc.facility) as facility_avg , avg(sc.`help`) as help_avg,
 avg(sc.clarity) as clarity_avg from scorings as sc inner join professors as pr on sc.professor_id = pr.professor_id group by pr.professor_id;
